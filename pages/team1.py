...
st.set_page_config(
    page_title="EcoCar Dashboard", layout="wide", initial_sidebar_state="expanded"
)
...
csv_path = os.path.join(current_dir, "..", "data", "team1-data.csv")

# Read the CSV file into a pandas DataFrame
df_reshaped = pd.read_csv(csv_path)
...
with st.sidebar:
    ...
    # Create a selectbox for color theme selection
    color_theme_list = [
        "blues",
        "cividis",
        "greens",
...
# Function to create a heatmap using Altair
...
# Function to create a choropleth map using Plotly
...
# Function to create a donut chart using Altair
...
# Function to format large numbers as text
...
# Function to calculate the year-over-year population difference
...
# Define the layout of the main panel using columns
col = st.columns((1.5, 4.5, 2), gap="medium")

with col[0]:
    st.markdown("#### Gains/Losses")

    # Calculate and display population differences for the selected year
    ...
    if selected_year > 2010:
        ...
        first_state_delta = ""
    st.metric(
        label=first_state_name, value=first_state_population, delta=first_state_delta
    )
...
with col[1]:
    ...
    # Display the choropleth map and heatmap
...
with col[2]:
    ...
    # Display the top states by population in a dataframe
    st.dataframe(
